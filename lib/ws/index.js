...
const Router = require('koa-router'),
    ...
    helpers = require('../util/dummyHelper')
...
function templateHandler(wsConfig) {
    ...
    return async ctx => {
        console.log(`${logPrefix}Middleware applied for ${endpoint}, type: ${type}`)
...
function generateJson(filePath) {
    const template = fs.readFileSync(absPath(filePath)).toString(),
        jsonStr = dummyJson.parse(template, { helpers })
    return jsonStr
}
